...
$sql = "SELECT hall_id, hall_name, capacity, price FROM halls 
        WHERE hall_id IN ('19', '20', '21', '22', '23', '24')";
...
$cateringOptions = [];
$sql = "SELECT catering_id, name, price FROM catering WHERE catering_id IN ('18','19','20')";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $cateringOptions[] = $row;
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($stmt->execute()) {
        echo "<script>alert('Booking successful!'); window.location.href='confirm.php';</script>";
        exit();
    } else {
...
<head>
    ...
    <title>Premium Booking Confirmation</title>
    <link rel="stylesheet" href="premium booking.css">
</head>
<body>
...
<main>
    <div class="booking">
        <h1 id="head">Our Premium Plan</h1>
