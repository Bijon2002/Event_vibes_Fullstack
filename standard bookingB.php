...
$cateringOptions = [];
$sql = "SELECT catering_id, name, price FROM catering WHERE catering_id IN ('4','5','6')";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $cateringOptions[] = $row;
...
<main>
    ...
    <h1 id="head">Our Standard Plan</h1>
        ...
        <form method="POST" >
            <input type="hidden" name="event_name" value="Birthday">
